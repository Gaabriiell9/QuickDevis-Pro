...
const HEADER_BG_H = 56; // height of BOLD/MODERN first-page background
...
export function generateQuotePdf(data: PdfQuoteData): void {
  ...
  // Side band width per STYLE
  const ACCENT_W = style === "CLASSIC" ? 6 : style === "MODERN" ? 4 : 0;

  // ── Helpers ───────────────────────────────────────────────────────────────

  /** Side accent band — redrawn on every new page. */
  const drawAccent = () => {
  ...
  /** Render logo (if allowed) or org name text. Returns new y. */
  ...
  if (headerStyle === "CENTERED") {
    ...
    if (style === "BOLD") {
      ...
      doc.text("DEVIS", PAGE_W / 2, y, { align: "center" });
      y += 7;
    } else {
      const BW = 44;
      ...
      doc.setFontSize(11);
      doc.setTextColor(C.white[0], C.white[1], C.white[2]);
      doc.text("DEVIS", PAGE_W / 2, y + 6.8, { align: "center" });
      y += 13;
    }
    ...
    doc.setFontSize(8);
    doc.setTextColor(TEXT_MUTED[0], TEXT_MUTED[1], TEXT_MUTED[2]);
    doc.text(`Émis le ${fDate(data.issueDate)}`, PAGE_W / 2, y, { align: "center" });
    y += 6;

  } else {
    // ── SPLIT or LEFT layout — org on left, badge on right ────────────────
    ...
    // Badge / title on right
    const badgeY = 14;
    if (style === "BOLD") {
      doc.setFont(FONT, "bold");
      doc.setFontSize(headerStyle === "LEFT" ? 16 : 18);
      doc.setTextColor(C.white[0], C.white[1], C.white[2]);
      doc.text("DEVIS", PAGE_W - MARGIN_R, badgeY + 7, { align: "right" });
      ...
      doc.setFontSize(9);
      doc.setTextColor(C.white[0], C.white[1], C.white[2]);
      doc.text("DEVIS", bx + BW / 2, badgeY + 5.5, { align: "center" });
    } else {
      // SPLIT
      const BW = 44;
      const bx = PAGE_W - MARGIN_R - BW;
      ...
      doc.setFontSize(11);
      doc.setTextColor(C.white[0], C.white[1], C.white[2]);
      doc.text("DEVIS", bx + BW / 2, badgeY + 6.8, { align: "center" });
    }
    ...
    doc.setFontSize(8);
    doc.setTextColor(TEXT_MUTED[0], TEXT_MUTED[1], TEXT_MUTED[2]);
    doc.text(`Émis le ${fDate(data.issueDate)}`, PAGE_W - MARGIN_R, badgeY + 21, { align: "right" });
  }
  ...
  if (data.validUntilDate) dateEntries.push(["Valide jusqu'au", fDate(data.validUntilDate)]);
  ...
  dateEntries.forEach(([label, value], i) => {
  ...
  if (tpl?.showSignatureBlock !== false) {
    ...
    const SIG_H = 36;
    const sig1X = MARGIN_L;
    const sig2X = MARGIN_L + SIG_W + 8;

    // Boîte gauche — Bon pour accord
    doc.setFillColor(C.bg[0], C.bg[1], C.bg[2]);
    doc.setDrawColor(C.border[0], C.border[1], C.border[2]);
    doc.setLineWidth(0.3);
    doc.roundedRect(sig1X, y, SIG_W, SIG_H, 2, 2, "FD");
    doc.setFont(FONT, "bold");
    doc.setFontSize(7);
    doc.setTextColor(C.muted[0], C.muted[1], C.muted[2]);
    doc.text("BON POUR ACCORD", sig1X + 4, y + 7);
    doc.setFont(FONT, "normal");
    doc.setFontSize(7);
    doc.setTextColor(C.subtle[0], C.subtle[1], C.subtle[2]);
    doc.text("Date : _____ / _____ / __________", sig1X + 4, y + 18);
    doc.setDrawColor(C.subtle[0], C.subtle[1], C.subtle[2]);
    doc.setLineWidth(0.3);
    doc.line(sig1X + 4, y + 30, sig1X + SIG_W - 4, y + 30);
    doc.setFontSize(6.5);
    doc.text("Signature", sig1X + 4, y + 34);

    // Boîte droite — Cachet
    doc.setFillColor(C.bg[0], C.bg[1], C.bg[2]);
    doc.setDrawColor(C.border[0], C.border[1], C.border[2]);
    doc.setLineWidth(0.3);
    doc.roundedRect(sig2X, y, SIG_W, SIG_H, 2, 2, "FD");
    doc.setFont(FONT, "bold");
    doc.setFontSize(7);
    doc.setTextColor(C.muted[0], C.muted[1], C.muted[2]);
    doc.text("CACHET ET SIGNATURE", sig2X + 4, y + 7);
  }
