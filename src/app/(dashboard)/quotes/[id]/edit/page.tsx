...
type QuoteEditForm = z.infer<typeof schema>;
...
export default function QuoteEditPage() {
  ...
  useEffect(() => {
    if (!quote || isInitialized.current) return;
    form.reset({
      clientId: quote.clientId ?? "",
      subject: quote.subject ?? "",
      issueDate: quote.issueDate ? quote.issueDate.split("T")[0] : "",
      validUntilDate: quote.validUntilDate ? quote.validUntilDate.split("T")[0] : "",
      notes: quote.notes ?? "",
  ...
  // Save function
  const save = useCallback(async (data: QuoteEditForm) => {
  ...
  const previewData: PreviewData = {
    ...
    validUntilDate: watchAll.validUntilDate,
    ...
    <div className="-m-6 h-[calc(100vh-4rem)] flex flex-col overflow-hidden bg-white">
      ...
      <header className="flex items-center justify-between h-14 px-5 border-b border-slate-100 bg-white shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          <Link href={`/quotes/${id}`} className="flex items-center gap-1.5 text-slate-500 hover:text-slate-900 text-sm transition-colors">
            <ArrowLeft className="size-4" />
            Retour
          </Link>
          <span className="text-slate-300">|</span>
          <span className="text-sm font-semibold text-slate-800 truncate">
            {quote?.reference ?? "Édition du devis"}
          </span>
          {savedLabel && (
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-1/2 overflow-y-auto border-r border-slate-100">
          <form onSubmit={form.handleSubmit(save)} className="divide-y divide-slate-100">
            ...
            {/* Titre */}
            <Section title="Titre du devis" open={openSections.title} onToggle={() => toggleSection("title")}>
              <div className="space-y-4">
                <div className="space-y-1.5">
                  <Label className="text-xs text-slate-500">Objet / titre</Label>
                  <Input {...form.register("subject")} placeholder="Ex: Refonte site web" className="h-10" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs text-slate-500">Date de validité</Label>
                    <Input type="date" {...form.register("validUntilDate")} className="h-10" />
                  </div>
                </div>
              </div>
            </Section>
