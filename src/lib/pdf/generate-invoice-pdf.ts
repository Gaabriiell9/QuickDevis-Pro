...
import type { PdfInvoiceData, TemplateConfig, PdfOrganization, PdfItem } from "./pdf-types";
...
function drawNotes(doc: jsPDF, data: PdfInvoiceData, y: number, FONT: string): number {
...
function drawBankDetails(
  ...
  if (!org.iban) return y;
  y = checkBreak(doc, y, 20);
  doc.setFont(FONT, "bold"); doc.setFontSize(7);
  doc.setTextColor(PRIMARY[0], PRIMARY[1], PRIMARY[2]);
  doc.text("COORDONNÉES BANCAIRES", ML, y); y += 4;
  doc.setFont(FONT, "normal"); doc.setFontSize(7.5);
  doc.setTextColor(C.muted[0], C.muted[1], C.muted[2]);
  doc.text(`IBAN : ${org.iban}`, ML, y); y += 3.8;
  if (org.bic) { doc.text(`BIC : ${org.bic}`, ML, y); y += 3.8; }
  return y + 4;
}
...
function drawPaymentBar(
  ...
  if (data.amountPaid <= 0 && data.status !== "OVERDUE") return y;

  if (style === "MINIMAL") {
    doc.setFont(FONT, "normal"); doc.setFontSize(8);
    doc.setTextColor(C.muted[0], C.muted[1], C.muted[2]);
    const parts: string[] = [];
    if (data.amountPaid > 0) parts.push(`Payé : ${fMoney(data.amountPaid, currency)}`);
    if (data.amountDue > 0) parts.push(`Reste dû : ${fMoney(data.amountDue, currency)}`);
  ...
  if (style === "BOLD") {
    const paidBg: [number, number, number] = data.amountPaid > 0 ? lighten(SECONDARY, 0.9) : C.bg;
    const paidFg: [number, number, number] = data.amountPaid > 0 ? SECONDARY : C.subtle;
    doc.setFillColor(paidBg[0], paidBg[1], paidBg[2]);
    doc.roundedRect(ML, y, half, BAR_H, 2, 2, "F");
    doc.setFont(FONT, "bold"); doc.setFontSize(7); doc.setTextColor(paidFg[0], paidFg[1], paidFg[2]);
    doc.text("PAYÉ", ML + 8, y + 5);
    doc.setFontSize(10); doc.text(fMoney(data.amountPaid, currency), ML + 8, y + 11);

    const dueBg: [number, number, number] = data.amountDue > 0 ? lighten(PRIMARY, 0.88) : C.bg;
    const dueFg: [number, number, number] = data.amountDue > 0 ? PRIMARY : C.subtle;
    const dueX = ML + half + 4;
    doc.setFillColor(dueBg[0], dueBg[1], dueBg[2]);
    doc.roundedRect(dueX, y, half, BAR_H, 2, 2, "F");
    doc.setFont(FONT, "bold"); doc.setFontSize(7); doc.setTextColor(dueFg[0], dueFg[1], dueFg[2]);
    doc.text("RESTE DÛ", dueX + 8, y + 5);
    doc.setFontSize(10); doc.text(fMoney(data.amountDue, currency), dueX + 8, y + 11);
  } else {
    // CLASSIC / MODERN
...
function renderClassicInvoiceLayout(doc: jsPDF, data: PdfInvoiceData, config: TemplateConfig): void {
  ...
  if (data.quoteReference) dateEntries.push(["Devis", data.quoteReference]);
  ...
  y = drawPaymentBar(doc, data, y, PRIMARY, SECONDARY, FONT, "CLASSIC", currency);
  ...
  if (config.showBankDetails && org.iban) {
    y = drawBankDetails(doc, org, y, FONT, PRIMARY);
  }
...
function renderModernInvoiceLayout(doc: jsPDF, data: PdfInvoiceData, config: TemplateConfig): void {
  const PRIMARY = hexToRgb(config.primaryColor ?? "#4338CA");
  const SECONDARY = hexToRgb(config.secondaryColor ?? "#1E293B");
  const FONT = config.fontStyle === "SERIF" ? "times" : "helvetica";
  const currency = "EUR";
  ...
  let dy = y + 4;
  const dateEntries: [string, string][] = [["Date d'émission", fDate(data.issueDate)]];
  if (data.dueDate) dateEntries.push(["Échéance", fDate(data.dueDate)]);
  if (data.quoteReference) dateEntries.push(["Devis", data.quoteReference]);
  dateEntries.forEach(([label, value]) => {
    doc.setFont(FONT, "normal"); doc.setFontSize(7);
    doc.setTextColor(C.subtle[0], C.subtle[1], C.subtle[2]);
  ...
  y = drawPaymentBar(doc, data, y, PRIMARY, SECONDARY, FONT, "MODERN", currency);
...
function renderMinimalInvoiceLayout(doc: jsPDF, data: PdfInvoiceData, config: TemplateConfig): void {
  const PRIMARY = hexToRgb(config.primaryColor ?? "#4338CA");
  const SECONDARY = hexToRgb(config.secondaryColor ?? "#1E293B");
  const FONT = config.fontStyle === "SERIF" ? "times" : "helvetica";
  const currency = "EUR";
  ...
  if (data.dueDate) datesStr += `   ·   Échéance : ${fDate(data.dueDate)}`;
  doc.setFont(FONT, "normal"); doc.setFontSize(7.5);
  doc.setTextColor(C.muted[0], C.muted[1], C.muted[2]);
  ...
  y += 4;
  y = drawPaymentBar(doc, data, y, PRIMARY, SECONDARY, FONT, "MINIMAL", currency);
...
// ── BOLD INVOICE LAYOUT ────────────────────────────────────────────────────────
function renderBoldInvoiceLayout(doc: jsPDF, data: PdfInvoiceData, config: TemplateConfig): void {
  const PRIMARY = hexToRgb(config.primaryColor ?? "#4338CA");
  const SECONDARY = hexToRgb(config.secondaryColor ?? "#1E293B");
  const FONT = config.fontStyle === "SERIF" ? "times" : "helvetica";
  const currency = "EUR";
  ...
  y = drawPaymentBar(doc, data, y, PRIMARY, SECONDARY, FONT, "BOLD", currency);
...
export function generateInvoicePdf(data: PdfInvoiceData): void {
  ...
  switch (style) {
    case "MODERN":  renderModernInvoiceLayout(doc, data, config);  break;
    case "MINIMAL": renderMinimalInvoiceLayout(doc, data, config); break;
    case "BOLD":    renderBoldInvoiceLayout(doc, data, config);    break;
    default:        renderClassicInvoiceLayout(doc, data, config); break;
  }
